...
import { loadTestChecklist, saveTestChecklist } from "../lib/testChecklist";

function TestChecklistPage() {
  const navigate = useNavigate();
  const [tests, setTests] = useState(() => loadTestChecklist().tests);
  const [completed, setCompleted] = useState(() => loadTestChecklist().completed);

  useEffect(() => {
    const { tests: loaded, completed: c } = loadTestChecklist();
    setTests(loaded);
    setCompleted(c);
  }, []);
  ...
  const handleCheckAll = () => {
    const allChecked = tests.map((t) => ({ ...t, checked: true }));
    updateTests(allChecked);
  };
        ...
        <div className="max-w-4xl mx-auto space-y-6">
          <header className="space-y-2">
            ...
            <p className="text-sm text-slate-300 max-w-2xl">
              Internal checklist to validate the Placement Readiness Platform
              before shipping. Results are stored locally on this device.
            </p>
          </header>
              ...
              {tests.map((test) => (
                ...
                >
                  ...
                  <div>
                    ...
                    {test.hint && (
                      <p className="mt-1 text-xs text-slate-400">{test.hint}</p>
                    )}
                  </div>
                </div>
              ))}
